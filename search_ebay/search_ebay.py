...
def search_ebay(self, keywords_to_search: str):
    ...
    """
    Search Ebay using keyword(s) for an item. Set heartbeat = True when running this function and display the results.
    ...
    def format_ebay_item(item):
        return (
            ...
            f"Category Name: {item['primaryCategory'][0]['categoryName'][0]}\n"
            #f"Gallery URL: {item['galleryURL'][0]}\n"
            f"View Item URL: {item['viewItemURL'][0]}\n"
            #f"AutoPay: {item['autoPay'][0]}\n"
            #f"Postal Code: {item['postalCode'][0]}\n"
            f"Location: {item['location'][0]}\n"
            f"Country: {item['country'][0]}\n"
            f"Shipping Service Cost (USD): {item['shippingInfo'][0]['shippingServiceCost'][0]['__value__']}\n"
            f"Shipping Type: {item['shippingInfo'][0]['shippingType'][0]}\n"
            ...
            f"Converted Current Price (USD): {item['sellingStatus'][0]['convertedCurrentPrice'][0]['__value__']}\n"
            ...
            f"Buy It Now Available: {item['listingInfo'][0]['buyItNowAvailable'][0]}\n"
            ...
            f"Condition Display Name: {item['condition'][0]['conditionDisplayName'][0]}\n"
